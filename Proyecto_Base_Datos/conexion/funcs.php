<?php
	...
	function isNull($nombre, $apellido, $password, $confirm_password, $email){
		if(strlen(trim($nombre)) < 1 || strlen(trim($apellido)) < 1 || strlen(trim($password)) < 1 || strlen(trim($confirm_password)) < 1 || strlen(trim($email)) < 1)
		{
			return true;
			} else {
			return false;
	...
	function emailExiste($email)
	{
		...
		$stmt = $mysqli->prepare("SELECT id FROM tbl_usuario WHERE 	Correo_Usuario = ? LIMIT 1");
	...
	function registraUsuario($nombre, $apellido, $pass_hash, $email, $activo, $token, $FK_Id_Genero){
		
		global $mysqli;
		
		$stmt = $mysqli->prepare("INSERT INTO tbl_usuarios (Prim_Nom_Usuario, Prim_Ape_Usuario, Contraseña_Usuario,	Correo_Usuario, activacion, token, FK_Id_Genero) VALUES(?,?,?,?,?,?,?)");
		$stmt->bind_param('ssssisi', $nombre, $apellido, $pass_hash, $email, $activo, $token, $FK_Id_Genero);
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Username = 'latuyaporsiacaso593@gmail.com'; //Modificar
		$mail->Password = 'Cuenta#12345'; //Modificar
		
		$mail->setFrom('latuyaporsiacaso593@gmail.com', 'Sistema de Usuario'); //Modificar
